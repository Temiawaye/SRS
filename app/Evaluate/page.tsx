                                ...
                                <div className="p-0 flex-1 overflow-y-auto relative srs-print-scroll">
                                    {isExternalMode ? (
                                        <div className="p-5 h-full flex flex-col gap-4">
                                            ...
                                            {/* Single unified input box: paste + drag-and-drop + upload */}
                                            <div
                                                className={`relative flex flex-col rounded-2xl border-2 transition-all duration-200 overflow-hidden ${isDragging
                                            ...
                                            >
                                                {/* Top bar: file pill (if loaded) + upload button */}
                                                <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                                                    {importedFileName ? (
                                                        <div className="flex items-center gap-2 min-w-0">
                                                            <svg className="w-3.5 h-3.5 text-emerald-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                                            <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 truncate">{importedFileName}</span>
                                                            <span className="text-xs text-slate-400 shrink-0">{(externalContent.length / 1024).toFixed(1)} KB</span>
                                                            <button
                                                        ...
                                                        <span className="text-xs text-slate-400 dark:text-slate-500">Paste text or upload a file below</span>
                                                    )}
                                                    <button
                                                        onClick={() => fileInputRef.current?.click()}
                                                        className="shrink-0 flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 border border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700 rounded-lg px-2.5 py-1.5 transition-all"
                                                        title="Upload file"
                                                    >
                                                ...
                                                {/* Textarea + centered instruction overlay */}
                                                <div className="relative min-h-[260px]">
                                                    {/* Centered instructions — shown only when box is empty */}
                                                    {!externalContent && !isDragging && (
                                                        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 select-none pointer-events-none">
                                                            <div
                                                                onClick={() => fileInputRef.current?.click()} className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center mb-1 cursor-pointer pointer-events-auto transition-colors"
                                                            >
                                                                <svg className="w-6 h-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                                </svg>
                                                            </div>
                                                            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Paste or drag & drop your document</p>
                                                            <p className="text-xs text-slate-400 dark:text-slate-500">Supports .pdf · .docx · .txt · .md</p>
                                                        </div>
                                                    )}
                                                    <textarea
                                                        className="relative w-full h-full p-4 text-xs leading-relaxed text-slate-700 dark:text-slate-300 bg-transparent resize-none font-mono focus:outline-none min-h-[260px] caret-emerald-500"
                                                        placeholder=""
                                                ...
                                                {/* Drag overlay */}
                                                {isDragging && (
                                                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-emerald-50/90 dark:bg-emerald-950/70 pointer-events-none">
                                                        <div className="w-14 h-14 rounded-2xl bg-emerald-100 dark:bg-emerald-900/60 flex items-center justify-center">
                                                            <svg className="w-7 h-7 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                                        </div>
                                                        <p className="font-semibold text-emerald-700 dark:text-emerald-300 text-sm">Drop to upload</p>
                                                    </div>
                                                )}

                                                {/* Parsing indicator */}
                                                {(isParsingDocx || isParsingPdf) && (
                                                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-white/80 dark:bg-slate-900/80 pointer-events-none">
                                                        <svg className="animate-spin w-5 h-5 text-emerald-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>
                                                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{isParsingDocx ? 'Reading Word document…' : 'Reading PDF…'}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
